<div id="custom-content" class="modal-block modal-block-md" style="max-width: 80% !important;">
    <section class="panel">
        <header class="panel-heading">
            ...
            <h2 class="panel-title">{{Help::languages('Edit')}} {{Help::languages('Job')}}</h2>
        </header>
        <div class="panel-body">
            {!! Form::open(['url'=>'jobs/'.$job->id,'method'=>'put','class'=>'form-horizontal','id'=>'form','files'=>'true','novalidate'=>'novalidate']) !!}
                <div class="form-group">
                    <label class="col-md-2 control-label">{{Help::languages('Code')}} <span class="required" >*</span></label>
                    <div class="col-md-9">
                        <input type="text" required="" placeholder="" value="{{$job->code}}" class="form-control" name="code" >
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-2 control-label">{{Help::languages('Name')}} <span class="required" >*</span></label>
                    <div class="col-md-9">
                        <input type="text" required="" placeholder="" value="{{$job->name}}" class="form-control" name="name" >
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-2 control-label">{{Help::languages('Description')}}</label>
                    <div class="col-md-9">
                        <textarea placeholder="" class="form-control" rows="5" name="description" >{{$job->description}}</textarea>
                    </div>
                </div>
            <div class="form-group">
